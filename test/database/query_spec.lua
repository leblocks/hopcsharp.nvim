...
describe('query', function()
    ...
    it('can invoke get_definition_by_name_and_type query', function()
        local db = database.__get_db()
        assert(db:eval(query.get_definition_by_name_and_type('name', 2)))
    end)
    ...
    it('can invoke get_definition_by_name_type_and_namespace', function()
        local db = database.__get_db()
        assert(db:eval(query.get_definition_by_name_type_and_namespace('test', 2, { 'Test.One', 'Test.Two' })))
    end)
end)

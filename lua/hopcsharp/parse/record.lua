...
M.__parse_records = function(tree, file_path, file_content, db)
    ...
    pautils.__icaptures(query.record_declaration, tree, file_content, function(node, content)
        pautils.__icaptures(query.record_identifier, node, content, function(n, c)
            local row, col, _, _ = n:range()
            local name = vim.treesitter.get_node_text(n, c, nil)
            dbutils.__insert_object(db, path_id, namespace_id, dbutils.__types.RECORD, name, row, col)
        end)
    end)
end

...
M.__parse_enums = function(tree, file_path, file_content, db)
    ...
    pautils.__icaptures(query.enum_declaration, tree, file_content, function(node, content)
        pautils.__icaptures(query.enum_identifier, node, content, function(n, c)
            local row, col, _, _ = n:range()
            local name = vim.treesitter.get_node_text(n, c, nil)
            dbutils.__insert_object(db, path_id, namespace_id, dbutils.__types.ENUM, name, row, col)
        end)
    end)
end
